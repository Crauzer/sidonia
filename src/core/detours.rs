...
use std::ffi;
use crate::core::riot::logger::{RiotLogger, RiotLogSeverityLevel, RiotLogTags};
...
pub unsafe fn initialize_init_renderer_hook(detour: InitRendererFn) -> Result<(), Box<dyn Error>> {
    ...
    InitRendererHook
        .initialize(address, detour)?
        .enable()?;

    Ok(())
}

pub unsafe fn initialize_riot_x3d_d3d9_device_end_scene_hook(
    detour: RiotX3dD3D9DeviceEndSceneFn
) -> Result<(), Box<dyn Error>> {
    let address = mem::transmute(memory::convert_file_offset_to_ptr(0x009C2500));

    RiotX3dD3D9DeviceEndSceneHook
        .initialize(address, detour)?
        .enable()?;
